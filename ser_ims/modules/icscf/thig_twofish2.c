...
#endif
#endif
#include	"thig_debug.h"				/**< debug display macros */
...
* 
* @param op	- what to do	(see TAB_* defns in AES.H)
* @returns  TRUE --> done (for TAB_QUERY)		
* 
****************************************************************************/
int TableOp(int op)
	{
...
/******************************************************************************
*
* Parse ASCII hex nibbles and fill in key/iv dwords
* Notes:  
...
* 
* @param bit    - # bits to read
* @param scrTxt - ASCII source
* @param d      - ptr to dwords to fill in
* 				  Note that the parameter d is a DWORD array, not a byte array.
* @param dstTxt - where to make a copy of ASCII source (NULL ok)
*
* @returns	Zero if no error.  Nonzero --> invalid hex or length
*
*
****************************************************************************/
...
* @param k32 	- pointer to key dwords
* @param keyLen - total key length (k32 --> keyLey/2 bits)
*
...
*
* @param key - ptr to keyInstance to be initialized
*
...
* @param keyLen			- # bits of key text at *keyMaterial
* @param keyMaterial	- ptr to hex ASCII chars representing key bits
*
* @returns - 			TRUE on success else error code (e.g., BAD_KEY_DIR)
*
* 
*****************************************************************************/
...
*
* @param cipher	- ptr to cipherInstance to be initialized
* @param mode   - MODE_ECB, MODE_CBC, or MODE_CFB1
* @param IV	    - ptr to hex ASCII test representing IV bytes
*
* @returns - TRUE on success else error code (e.g., BAD_CIPHER_MODE)
*
-****************************************************************************/
...
*
* @param cipher 	- ptr to already initialized cipherInstance
* @param key    	- ptr to already initialized keyInstance
* @param input		- ptr to data blocks to be encrypted
* @param inputLen	- # bits to encrypt (multiple of blockSize)
*					outBuffer	=	ptr to where to put encrypted blocks
*
* @returns			# bits ciphered (>= 0)
...
* @param cipher 	- ptr to already initialized cipherInstance
* @param key 		- ptr to already initialized keyInstance
* @param input		- ptr to data blocks to be decrypted
* @param inputLen	- # bits to encrypt (multiple of blockSize)
* @param outBuffer	- ptr to where to put decrypted blocks
*
* @returns			# bits ciphered (>= 0)
*					else error code (e.g., BAD_CIPHER_STATE, BAD_KEY_MATERIAL)
*
